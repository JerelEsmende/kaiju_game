...
public class IndvShip2 : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (turning)
        {
            ...
            transform.rotation = Quaternion.Slerp(transform.rotation, Quaternion.LookRotation(direction), rotationSpd * Time.deltaTime);
            spd = Random.Range(5f, 5.5f);
        }
        else
        {
    ...
    void newRegulations()
    {
        ...
        ships2 = GlobArmada.allFleet2;

        Vector3 center = Vector3.zero;
        Vector3 socDist = Vector3.zero;

        Vector3 goal = GlobArmada.goal2;
        float gSpd = 0.1f;
        ...
        foreach (GameObject ship in ships2)
        {
            if (ship != this.gameObject)
            {
                ...
                if (distance <= neighborDist)
                {
                    ...
                    IndvShip2 otherShip = ship.GetComponent<IndvShip2>();
