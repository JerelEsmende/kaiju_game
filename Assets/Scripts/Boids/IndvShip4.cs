...
public class IndvShip4 : MonoBehaviour
{
    ...
    void newRegulations()
    {
        ...
        ships4 = GlobArmada.allFleet4;

        Vector3 center = Vector3.zero;
        Vector3 socDist = Vector3.zero;

        Vector3 goal = GlobArmada.goal4;
        float gSpd = 0.1f;
        ...
        foreach (GameObject ship in ships4)
        {
            if (ship != this.gameObject)
            {
                ...
                if (distance <= neighborDist)
                {
                    ...
                    IndvShip4 otherShip = ship.GetComponent<IndvShip4>();
